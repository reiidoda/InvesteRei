...
@Service
public class HttpPushNotificationProvider implements NotificationProvider {
  private static final Logger log = LoggerFactory.getLogger(HttpPushNotificationProvider.class);

  private final NotificationPushProperties properties;
  private final RestTemplate restTemplate;
  private final ObjectMapper mapper;

  public HttpPushNotificationProvider(NotificationPushProperties properties,
  ...
  public boolean supports(String providerId, NotificationChannel channel) {
    ...
    return normalized.equals("http-push") || normalized.equals("push-http") || normalized.equals("push");
  }
  ...
  public NotificationProviderResult send(NotificationChannel channel,
    ...
    if (!properties.isEnabled()) {
      return NotificationProviderResult.skipped(providerId, "push disabled");
    }
    if (properties.getBaseUrl() == null || properties.getBaseUrl().isBlank()) {
      return NotificationProviderResult.skipped(providerId, "push baseUrl not configured");
    }
    if (destination == null || destination.getDestination() == null || destination.getDestination().isBlank()) {
      return NotificationProviderResult.failed(providerId, "push destination missing");
    }

    URI uri = buildUri(properties.getBaseUrl(), properties.getSendPath());
    if (uri == null) {
      return NotificationProviderResult.failed(providerId, "invalid push endpoint");
    }
    ...
    payload.put("token", destination.getDestination());
    ...
    try {
      ...
      if (code == 404 || code == 410) {
        return NotificationProviderResult.bounced(providerId, "push endpoint not found");
      }
      return NotificationProviderResult.failed(providerId, "push status " + code);
